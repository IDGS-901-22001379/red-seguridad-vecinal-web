// src/pages/finanzas/CargosMantenimiento.jsx
import { useEffect, useMemo, useState } from "react";
import PagosAPI from "../../services/pagos.api";
import { UsuariosAPI } from "../../services/usuarios.api"; // <- tu export nombrado
...
export default function CargosMantenimiento() {
  const [rows, setRows] = useState([]); // filas ya enriquecidas con usuario
  ...
  // Carga todos los cargos para todos los usuarios
  const loadAll = async () => {
    setErr("");
    setLoading(true);
    try {
      ...
      const tareas = usuarios.map(async (u) => {
        ...
        if (!userId) return [];
        const cargos = await PagosAPI.getCargosMantenimiento(Number(userId));
      ...
      setErr(e.message || "No se pudieron obtener los cargos.");
      setRows([]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="p-4 md:p-6">
      ...
      <div className="mb-4">
        ...
        <p className="text-slate-500 text-sm">
          Vista de administrador: todos los cargos de mantenimiento por usuario.
        </p>
      </div>
      ...
      {/* Tabla sin encabezado “Resultados”, con barras verde/verde claro */}
        ...
        <div className="overflow-x-auto bg-white">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              ...
              {dataFiltrada.map((r) => (
                <tr
                  key={`${r.cargoMantenimientoID}-${r.__usuarioId}`}
